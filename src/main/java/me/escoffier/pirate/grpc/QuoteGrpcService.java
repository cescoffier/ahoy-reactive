package me.escoffier.pirate.grpc;

import io.smallrye.mutiny.Uni;
import me.escoffier.pirate.MutinyQuoteServiceGrpc;
import me.escoffier.pirate.QuoteReply;
import me.escoffier.pirate.QuoteRequest;
...
@Singleton
public class QuoteGrpcService extends MutinyQuoteServiceGrpc.QuoteServiceImplBase {
    Random random = new Random();

    @Override
    public Uni<QuoteReply> getRandomQuote(QuoteRequest request) {
        int i = random.nextInt(quotes.size());
        return Uni.createFrom().item(QuoteReply.newBuilder().setQuote(quotes.get(i)).build());
    }
